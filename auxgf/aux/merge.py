...
def aux_merge_exact(se, etol=1e-10, wtol=1e-12):
    ...
    if etol == 0:
        ...
        e_round = np.around(se.e, decimals=-int(np.log10(etol)))

    e_uniq, inds, cts = np.unique(e_round, return_index=True, return_counts=True)

    if e_uniq.shape == (se.naux,):
        return se
    ...
    v_list = [se.v[:,i:i+c] for i,c in zip(inds_degen, cts_degen)]
    ...
    e_b = np.repeat(se.e[inds_degen], [x.shape[1] for x in l])
    ...
    assert v_a.shape == (se.nphys, e_a.shape[0])
    assert v_b.shape == (se.nphys, e_b.shape[0])
