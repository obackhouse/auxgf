...
def _legendre_bath_kernel_1(e, beta, chempot=0.0):
    ...
    x1 = 1.0*x0
    x2 = np.exp(x0)
    val = -(x1*x2 + x1 - 2.0*x2 + 2.0)*np.exp(-x0*np.heaviside(a-chempot, 0.0))/(a**2*b)
    return val
...
def _legendre_bath_kernel_2(e, beta, chempot=0.0):
    ...
    x3 = np.exp(x1)
    x4 = 1.0*a**2*x0
    val = -1.0*(x2*x3 + x2 - x3*x4 - 12.0*x3 + x4 + 12.0)*np.exp(-x1*np.heaviside(a-chempot, 0.0))/(a**3*x0)
    return val
...
def _legendre_bath_kernel_3(e, beta, chempot=0.0):
    ...
    x4 = 12.0*a**2*b**2
    x5 = 1.0*a**3*x0
    val = -(x2*x3 + x2 - x3*x4 + x3*x5 - 120.0*x3 + x4 + x5 + 120.0)*np.exp(-x1*np.heaviside(a-chempot, 0.0))/(a**4*x0)
    return val
...
def _legendre_bath_kernel_4(e, beta, chempot=0.0):
    ...
    b = beta
    x0 = b**4
    x1 = np.heaviside(a-chempot, 0.0)
    x2 = a*b
    x3 = 20.0*a**3*b**3
    x4 = 840.0*x2
    x5 = 1.0*a**4*x0 + 180.0*a**2*b**2 + 1680.0
    val = ((-x3 - x4 + x5)*np.exp(x1*x2) - (x3 + x4 + x5)*np.exp(x2*(x1 - 1)))*np.exp(-x2*(2*x1 - 1))/(a**5*x0)
    return val
...
def _legendre_bath_kernel_5(e, beta, chempot=0.0):
    ...
    x3 = 30.0*a**4*b**4
    x4 = 3360.0*a**2*b**2
    x5 = 1.0*a**5*x0 + 420.0*a**3*b**3 + 15120.0*x2
    val = -((-x3 - x4 + x5 - 30240.0)*np.exp(x1*x2) + (x3 + x4 + x5 + 30240.0)*np.exp(x2*(x1 - 1)))*np.exp(-x2*(2*x1 - 1))/(a**6*x0)
    return val
...
def _legendre_bath_kernel_6(e, beta, chempot=0.0):
    ...
    val = ((-x3 - x4 - x5 + x6)*np.exp(x1*x2) - (x3 + x4 + x5 + x6)*np.exp(x2*(x1 - 1)))*np.exp(-x2*(2*x1 - 1))/(a**7*x0)
    return val
...
def _legendre_bath_kernel_7(e, beta, chempot=0.0):
    ...
    x3 = 56.0*a**6*b**6
    x4 = 25200.0*a**4*b**4
    x5 = 1995840.0*a**2*b**2
    x6 = 1.0*a**7*x0 + 1512.0*a**5*b**5 + 277200.0*a**3*b**3 + 8648640.0*x2
    val = -((-x3 - x4 - x5 + x6 - 17297280.0)*np.exp(x1*x2) + (x3 + x4 + x5 + x6 + 17297280.0)*np.exp(x2*(x1 - 1)))*np.exp(-x2*(2*x1 - 1))/(a**8*x0)
    return val
...
def _legendre_bath_kernel_8(e, beta, chempot=0.0):
    ...
    val = ((-x3 - x4 - x5 - x6 + x7)*np.exp(x1*x2) - (x3 + x4 + x5 + x6 + x7)*np.exp(x2*(x1 - 1)))*np.exp(-x2*(2*x1 - 1))/(a**9*x0)
    return val
